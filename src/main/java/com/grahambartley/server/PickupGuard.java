...
public final class PickupGuard {
	...
	public boolean tryNotify(ServerPlayerEntity player, ItemStack stack, boolean deleted, long currentTick) {
	...
	private boolean tryNotify(UUID playerUuid, ItemStack stack, boolean deleted, long currentTick, @Nullable ServerPlayerEntity player) {
		...
		if (player != null) {
			String message = deleted
				? String.format("§7[LootLock] §cDeleted %dx %s", stack.getCount(), stack.getItem().getName().getString())
				: String.format("§7[LootLock] §eBlocked %dx %s", stack.getCount(), stack.getItem().getName().getString());
