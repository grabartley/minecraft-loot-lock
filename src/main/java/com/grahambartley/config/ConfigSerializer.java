...
public final class ConfigSerializer {
	...
	public static LootLockPlayerData deserialize(String json, UUID expectedPlayerUuid) throws ConfigDeserializationException {
		try {
	...
	static LootLockPlayerData deserializeFromTree(JsonObject root, UUID expectedPlayerUuid) throws ConfigDeserializationException {
		...
		if (root.has("playerUuid")) {
			try {
				...
				if (expectedPlayerUuid != null && !fileUuid.equals(expectedPlayerUuid)) {
					throw new ConfigDeserializationException("Player UUID mismatch: expected " + expectedPlayerUuid + ", got " + fileUuid);
				}
		...
		if (root.has("activeProfileId")) {
			try {
				...
				throw new ConfigDeserializationException("Invalid activeProfileId format: " + e.getMessage(), e);
			}
		}
	...
	private static LootLockProfile deserializeProfile(JsonObject obj) throws ConfigDeserializationException {
		UUID id = null;
		if (obj.has("id")) {
			try {
		...
		if (obj.has("mode")) {
			try {
				...
				throw new ConfigDeserializationException("Invalid filter mode: " + obj.get("mode").getAsString(), e);
			}
		}
		...
		if (obj.has("rejectedItemAction")) {
			try {
				...
				throw new ConfigDeserializationException("Invalid rejectedItemAction: " + obj.get("rejectedItemAction").getAsString(), e);
			}
		}
