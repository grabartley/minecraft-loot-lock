...
public final class ConfigMigration {
	...
	public static ConfigValidationResult run(LootLockPlayerData data) {
		...
		if (schemaVersion > LootLockPlayerData.CURRENT_SCHEMA_VERSION) {
			warnings.add("Unknown schema version " + schemaVersion + ". Expected " + LootLockPlayerData.CURRENT_SCHEMA_VERSION + " or earlier.");
		...
		return warnings.isEmpty() ? ConfigValidationResult.success() : new ConfigValidationResult(true, warnings);
	}

	private static void migrateFromOlder(LootLockPlayerData data, int fromVersion, List<String> warnings) {
